...
export default function detail() {
  ...
  useEffect(() => {
    axios
      .post("https://157.90.95.45:97/api/Project/GetProjectDetail", {
        Id: param.id,
      })
      .then((result) => {
          ...
          <div className={style.container}>
            <div className={style.imageContainer}>
              ...
              <div className={style.tagContainer}>
                ...
                >
                  {detail.projectSubject == undefined
                    ? null
                    : detail.projectSubject.map((item) => {
                        return (
                          <span
                            style={{
                              padding: "0.2rem",
            ...
            >
              <div>
                <h3>{detail.projectName.toUpperCase()}</h3>
                <hr />
              </div>
              <div
                style={{
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <div>
                    {detail.projectIndustry.map((item, index) => {
                      const isLastItem =
                        index === detail.projectIndustry.length - 1;
                      return (
                        <span key={index}>
                          {item.industry_name}
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <div>
                    ...
                    {detail.projectGrades.map((item, index) => {
                      const isLastItem =
                        index === detail.projectGrades.length - 1;
                      return (
                        <span key={index}>
                          {item.grade_name}
                          {!isLastItem && <span>, </span>}
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <div>
                    ...
                    {detail.projectRole.map((item, index) => {
                      const isLastItem =
                        index === detail.projectRole.length - 1;
                      return (
                        <span key={index}>
                          {item.role_name}
                          {!isLastItem && <span>, </span>}
